...
long add_extrapolated_values(vector<vector<long>>& sequences) {
    ...
    for (int i = sequences.size() - 1; i > 0; i--) {
        long first_term = sequences[i].back();
        long second_term = sequences[i - 1].back();
        long extrapolated_term = first_term + second_term;
        sequences[i-1].push_back(extrapolated_term);
